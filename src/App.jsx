...
function Dialog({ kind, close, presence, notify, flowers }) {
  ...
  const heading = kind === 'release' ? 'What can you let go of?' : kind === 'hearth' ? 'You’re in good company.' : kind === 'garden' ? 'Let stillness take root.' : 'A softer place to land.';
  ...
  return <dialog className={`haven-dialog ${kind === 'help' ? 'help-dialog' : ''}`} ref={ref} aria-labelledby="dialog-title" onCancel={event => { event.preventDefault(); close(); }} onClick={event => { if (event.target === ref.current) { const r = ref.current.getBoundingClientRect(); if (event.clientX < r.left || event.clientX > r.right || event.clientY < r.top || event.clientY > r.bottom) close(); } }}>
    ...
    {kind === 'release' && <form onSubmit={release}>
      ...
      <button className="primary-button" type="submit" disabled={!thought.trim() || busy}>{busy ? 'Giving it to the river…' : 'Release this thought'}<Send size={15} strokeWidth={1.4} /></button>
      <p className="privacy-note"><ShieldCheck size={13} /> Your words disappear. Only the light is shared.</p>
    </form>}
    {kind === 'hearth' && <div className="dialog-content"><p>This fire grows a little warmer as people find their way here. No introductions needed. Just a shared moment.</p><div className="hearth-illustration"><Flame size={54} strokeWidth={0.8} /><span /><span /><span /></div><p className="italic-note">Somewhere, someone is slowing down with you.</p><button className="primary-button" onClick={close}>Stay by the fire<ArrowDown size={15} /></button><p className="privacy-note">No names. No numbers. Just a little warmth.</p></div>}
...
export default function App() {
  ...
  return <main className={`haven-app ${lighting === 'night' ? 'is-night' : ''}`}>
    ...
    <section className="intro" key={view} aria-label="Welcome to your sanctuary">
      ...
      <p>{copy.body}</p>
      <div className="presence-note"><span className={`presence-dot ${presence.status}`} />{presence.status === 'connected' ? 'Here, together. Quietly.' : presence.status === 'connecting' ? 'Finding a quiet connection…' : 'Reconnecting. Your quiet space is still here.'}</div>
      {presence.incomingLantern && <button className="incoming-kindness" onClick={() => presence.support(presence.incomingLantern.id)} aria-label="Send kindness to a passing lantern"><Sparkles size={13} /> A passing light. Send a little kindness <span>↗</span></button>}
    </section>
